...
// Plugin registration.
	...
	$.fn.sortable = function (options) {
		...
		if (filtered.length && options && options.group) {
			let dummy = new Sortable(filtered, options);
		}
		else {
			filtered.each(function (ix, el) {
				let dummy = new Sortable(el, options);
			});
		}
		return this;
	};
	...
	$.fn.draggable = function (options) {
		if (options === 'destroy') {
			...
			this.not('.draggable').each(function (ix, el) {
				let dummy = new Draggable(el, options);
			});
		}
		return this;
	};
	...
	$.fn.droppable = function (options) {
		if (options === 'destroy') {
			...
			this.not('.droppable').each(function (ix, el) {
				let dummy = new Droppable(el, options);
			});
		}
		return this;
	};
	...
	$.fn.dragaware = function (options) {
		if (options === 'destroy') {
			...
			this.not('.dragaware').each(function (ix, el) {
				let dummy = new Dragaware(el, options);
			});
		}
		return this;
	};
